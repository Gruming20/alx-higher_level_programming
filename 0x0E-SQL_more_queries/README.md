...
states description:
id id can be different, as per the example)
...
You can use only one SELECT statement
The database name will be passed as an argument of the mysql command INT unique, auto generated, can't be null and is a primary key
name VARCHAR(256) can't be null
...
The states table contains only one record where name = California (but the
